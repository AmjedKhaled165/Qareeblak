            ...
            >
                ...
                <div className="p-4 bg-gradient-to-l from-emerald-600 to-teal-600 dark:from-emerald-700 dark:to-teal-800 text-white shadow-sm">
                    <div className="flex items-center justify-between mb-3">
                        <button onClick={onClose} className="p-2 hover:bg-white/20 rounded-xl transition" title="إغلاق المحادثة" aria-label="إغلاق نافذة المحادثة">
                            <X className="w-5 h-5" />
                        </button>
                        <span className="text-xs font-bold bg-white/20 px-3 py-1 rounded-full backdrop-blur-sm">
                            محادثة #{String(consultation.id).slice(-6)}
                        </span>
                    </div>

                    <div className="flex items-center gap-3">
                        <div className="w-11 h-11 bg-white/20 rounded-2xl flex items-center justify-center shrink-0 backdrop-blur-sm">
                            <User className="w-6 h-6" />
                        </div>
                        <div>
                            <h3 className="font-bold text-base sm:text-lg leading-tight">{consultation.customer_name || 'عميل'}</h3>
                            {consultation.customer_phone && (
                                <a href={"tel:" + consultation.customer_phone} className="flex items-center gap-1 text-xs opacity-90 hover:opacity-100 mt-0.5">
                ...
                <div className="bg-amber-50 dark:bg-amber-950/40 border-b border-amber-200 dark:border-amber-800/50 px-4 py-2.5 text-[12px] text-amber-800 dark:text-amber-300 flex items-center gap-2 font-bold dir-rtl">
                    <span>🛡️ محادثة آمنة: يمنع تبادل أرقام الهواتف أو روابط التواصل الخارجي لحمايتك وضمان تقديم الخدمة/السكن.</span>
                </div>

                {/* Messages Area */}
                <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-slate-50/80 dark:bg-slate-950/90">
                    {isLoading ? (
