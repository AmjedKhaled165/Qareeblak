...
import { LayoutDashboard, ShoppingBag, Star, TrendingUp, Settings, LogOut, Utensils, Plus, Trash2, Edit, Check, X, Clock, Camera, Upload, User, MessageSquare, Calendar, PackagePlus, Loader2, Package } from "lucide-react";
...
import { apiCall, bookingsApi, servicesApi, providerOrdersApi } from "@/lib/api";
...
export default function ProviderDashboard() {
    ...
    return (
        <div className="min-h-screen bg-background text-foreground dir-rtl flex transition-colors duration-500">
                                            ...
                                            <div className="space-y-3 text-right">
                                                ...
                                                {serviceForm.image && (
                                                    <div className="relative w-full h-48 rounded-2xl overflow-hidden border-2 border-border bg-background shadow-inner">
                                                        <img src={serviceForm.image} alt="معاينة" className="w-full h-full object-contain bg-background" />
                                                        <button
                                                            type="button"
                                    ...
                                    <Card key={service.id} className="group hover:border-primary/50 transition-all overflow-hidden bg-card border-border/50 rounded-[2rem] shadow-xl flex flex-col">
                                        {service.image && (
                                            <div className="h-44 w-full overflow-hidden bg-muted relative">
                                                <img src={service.image} alt={service.name} loading="lazy" decoding="async" className="w-full h-full object-contain object-center" />
                            ...
                            <div className="flex items-center justify-between flex-wrap gap-4">
                                ...
                                <div className="flex items-center gap-3">
                                    <Button
                                        ...
                                        className="bg-primary hover:bg-primary/90 text-white rounded-xl font-bold font-cairo gap-2 h-11 px-5 shadow-lg shadow-primary/20 transition-all hover:scale-[1.02] active:scale-95"
                                    >
                                                        ...
                                                        >
                                                            ...
                                                            <td className="px-8 py-6 flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                                                                {booking.status === 'confirmed' && (
                                                                    <Button
                                                                        ...
                                                                        onClick={async (e) => {
                                                                            ...
                                                                            try {
                                                                                await providerOrdersApi.updateStatus(booking.id, 'ready');
                                                                                toast('تم التجهيز — الطلب جاهز للاستلام من المندوب ✅', 'success');
                                                                                setOptimisticStatuses(prev => new Map(prev).set(booking.id, 'completed'));
                                                                                if (providerId) fetchPaginatedBookings();
                                                                            } catch (err: any) {
                                                                                toast(err?.message || 'حدث خطأ في تحديث الحالة', 'error');
                                                                            } finally {
                                                                                setUpdatingStatusId(null);
                                                                            }
                                                                        }}
                                                                        className="bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold font-cairo gap-1.5 text-xs h-9 px-4 shadow-md transition-all hover:scale-[1.02] active:scale-95"
                                                                    >
                                                                ...
                                                                {booking.status === 'completed' && (
                                                                    <span className="text-xs font-bold text-emerald-500 font-cairo">✅ جاهز</span>
                                                                )}
            ...
            {/* Provider Create Order Modal */}
            <Dialog open={isCreateOrderOpen} onOpenChange={setIsCreateOrderOpen}>
                <DialogContent className="sm:max-w-[600px] rounded-[2rem] bg-card border-border/50 shadow-2xl p-0 overflow-hidden font-cairo">
                    <DialogHeader className="p-6 pb-4 border-b border-border/50 bg-muted/20">
                        ...
                        <DialogDescription className="text-muted-foreground font-medium mt-1">
                            قم بإضافة عناصر الطلب وأسعارها ليتم تجهيزها للمندوب.
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="p-6 space-y-6 max-h-[60vh] overflow-y-auto">
                            ...
                            <div key={index} className="flex flex-col md:flex-row gap-4 p-4 rounded-xl border border-border/50 bg-muted/10 relative group">
                                <div className="flex-1 space-y-2">
                                    ...
                                    <Input 
                                        ...
                                        value={item.name}
                                        onChange={(e) => {
                                            const newItems = [...createOrderItems];
                                            newItems[index].name = e.target.value;
                                            setCreateOrderItems(newItems);
                                        }}
                                        className="h-11 rounded-xl bg-background border-border"
                                    />
                                </div>
                                ...
                                {createOrderItems.length > 1 && (
                                    <button 
                                        onClick={() => {
                                            const newItems = createOrderItems.filter((_, i) => i !== index);
                                            setCreateOrderItems(newItems);
                                        }}
                        ...
                        <Button 
                            variant="outline" 
                            onClick={() => setCreateOrderItems([...createOrderItems, { name: '', price: '', quantity: '1' }])}
                        ...
                        <div className="flex justify-between items-center p-4 rounded-xl bg-primary/10 border border-primary/20">
                            ...
                            <span className="text-xl font-black text-primary">
                                {createOrderItems.reduce((sum, item) => sum + (Number(item.price || 0) * Number(item.quantity || 1)), 0).toFixed(2)} ج.م
                            </span>
                        </div>
                    </div>
                    ...
                    <DialogFooter className="p-6 border-t border-border/50 bg-muted/20 flex gap-3 sm:justify-start">
                        <Button 
                            ...
                            disabled={isCreatingOrder || createOrderItems.some(i => !i.name.trim() || !i.price || Number(i.price) <= 0)}
                            onClick={async () => {
                                setIsCreatingOrder(true);
                                try {
                                    const items = createOrderItems.map(i => ({
                                        name: i.name,
