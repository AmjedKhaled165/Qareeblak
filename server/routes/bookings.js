    ...
    try {
        ...
        const result = await db.query(
            `INSERT INTO bookings 
             ...
             RETURNING id`,
            [userId, providerId, serviceId, userName, serviceName, providerName, price, details, JSON.stringify(items || []), status, bundleId, appointmentDate || null, appointmentType || 'immediate']
        );
...
router.get('/provider/:providerId', async (req, res) => {
    try {
        ...
        // Optimized query: Only essential columns, paginated
        const result = await db.query(`
            SELECT 
                id, 
                user_name, 
...
router.patch('/:id/status', async (req, res) => {
    try {
        ...
        // No-Rejection Policy: Manual orders (created by admins) cannot be rejected by providers
        if (status === 'rejected') {
            const bookingCheck = await db.query('SELECT service_name, halan_order_id FROM bookings WHERE id = $1', [id]);
            if (bookingCheck.rows.length > 0) {
                const svcName = bookingCheck.rows[0].service_name || '';
                const hasHalanOrder = !!bookingCheck.rows[0].halan_order_id;
                if (svcName.includes('طلب يدوي') || hasHalanOrder) {
                    return res.status(403).json({ error: 'لا يمكن رفض الطلبات اليدوية. تواصل مع الإدارة للإلغاء.' });
                }
            }
        }
        ...
        // Fetch to get parent_order_id for socket broadcast
        const bookingInfo = await db.query('SELECT parent_order_id, halan_order_id, provider_id FROM bookings WHERE id = $1', [id]);

        if (io) {
            io.emit('booking-updated', {
                id,
                status,
                parentId: bookingInfo.rows[0]?.parent_order_id
            });
        }

        // ==========================================
        // SYNC WITH DELIVERY ORDER
        // GUARD CLAUSE: For MANUAL orders, do PURE status update only.
        // For APP orders, allow auto-assign logic.
        // ==========================================
        ...
        if (bookingInfo.rows.length > 0 && bookingInfo.rows[0].halan_order_id) {
            ...
            // Check if this is a manual order (courier already assigned by creator)
            const orderTypeCheck = await db.query(
                'SELECT order_type, courier_id, source FROM delivery_orders WHERE id = $1',
                [halanOrderId]
            );
            const isManualOrder = orderTypeCheck.rows.length > 0 && (
                orderTypeCheck.rows[0].order_type === 'manual' ||
                (orderTypeCheck.rows[0].source && !orderTypeCheck.rows[0].source.includes('qareeblak'))
            );
            const alreadyHasCourier = orderTypeCheck.rows.length > 0 && !!orderTypeCheck.rows[0].courier_id;

            // STATUS MAPPING:
            // confirmed (Provider Accepted) -> keep 'pending' (waiting for preparation)
            // completed (Provider Ready) -> 'ready_for_pickup' (Driver Notified)
            // cancelled -> 'cancelled'

            let halanStatus = null;
            if (status === 'confirmed') halanStatus = 'pending'; // Confirmed does NOT assign driver yet
            if (status === 'completed') halanStatus = 'ready_for_pickup'; // Ready -> notify courier
            if (status === 'cancelled') halanStatus = 'cancelled';

            if (halanStatus) {
                if (isManualOrder || alreadyHasCourier) {
                    // ═══════════════════════════════════════════════════════
                    // MANUAL ORDER: Pure status update ONLY. No create_order, no auto-assign.
                    // The courier is ALREADY assigned (the creator or selected courier).
                    // ═══════════════════════════════════════════════════════
                    console.log(`[Booking] Manual/Assigned order #${halanOrderId}. Pure status update to '${halanStatus}'.`);
                    await db.query(
                        'UPDATE delivery_orders SET status = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2',
                        [halanStatus, halanOrderId]
                    );
                    if (io) io.emit('order-status-changed', { orderId: halanOrderId, status: halanStatus });
                } else {
                    // ═══════════════════════════════════════════════════════
                    // APP ORDER: Auto-assign courier when all providers are ready
                    // ═══════════════════════════════════════════════════════
                    if (status === 'completed') {
                        console.log(`[Booking] App order #${halanOrderId} ready. Triggering auto-assign...`);
                        try {
                    ...
                    // If not handled by auto-assign, update manually
                    if (halanStatus) {
                        await db.query(
                            'UPDATE delivery_orders SET status = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2',
                            [halanStatus, halanOrderId]
