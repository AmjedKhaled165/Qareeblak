...
export function StatCard({ title, value, icon: Icon, type, onClick, trend }: StatCardProps) {
    ...
    return (
        <motion.div
            ...
            onClick={onClick}
            className={`bg-white rounded-2xl p-6 border border-slate-100 transition-all ${
                onClick ? 'cursor-pointer hover:border-slate-200' : ''
            }`}
            style={{
                boxShadow: LIGHT_MODE_COLORS.shadows.card,
            }}
        >
            {/* Header with Icon */}
...
export function DashboardHeader({ userName, onSettings, onLogout, onRefresh, isRefreshing }: HeaderProps) {
    return (
        <div
            className="bg-white border-b border-slate-100 px-6 py-4 shadow-sm"
            style={{ boxShadow: LIGHT_MODE_COLORS.shadows.subtle }}
        >
...
export function SectionCard({ title, icon: Icon, children, action }: SectionCardProps) {
    return (
        <div
            ...
            style={{ boxShadow: LIGHT_MODE_COLORS.shadows.card }}
        >
            {/* Header */}
            <div
...
export function BottomNavBar({ items }: BottomNavBarProps) {
    return (
        <div
            className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 px-4 py-3 flex justify-between items-center"
            style={{
                boxShadow: `${LIGHT_MODE_COLORS.shadows.subtle}, 0 -2px 8px rgba(0,0,0,0.02)`,
            }}
            dir="rtl"
        >
            {items.map((item, idx) => (
