...
router.get('/', authenticatePartner, async (req, res) => {
    try {
        ...
        const countQueryText = `SELECT COUNT(*) FROM delivery_orders o ${conditions.length > 0 ? 'WHERE ' + conditions.join(' AND ') : ''}`;
...
router.get('/courier', authenticatePartner, async (req, res) => {
    try {
        ...
        // Get orders:
        // 1. Assigned to this courier (both app & manual)
        // 2. Unassigned APP orders (pending/ready) - couriers can claim these
        // 3. NOT unassigned MANUAL orders - those belong to the creator/admin
        const result = await pool.query(`
            SELECT * FROM delivery_orders 
            WHERE (
                courier_id = $1 
                OR (courier_id IS NULL AND status IN ('pending', 'ready_for_pickup') AND (order_type = 'app' OR order_type IS NULL))
            )
...
router.get('/:id', authenticatePartner, async (req, res) => {
    try {
        ...
        // Fetch sub-orders (bookings) if they exist
        // We check for bookings that reference this delivery order (halan_order_id)
        // OR if this is a parent order, we might need to check by parent_order_id.
        // The implementation plan says "halan_order_id".
        const subOrdersRes = await pool.query(`
            SELECT b.id, b.provider_id, b.provider_name, b.status, b.items, b.price
            FROM bookings b
            WHERE b.halan_order_id = $1
        `, [id]);

        if (subOrdersRes.rows.length > 0) {
            order.sub_orders = subOrdersRes.rows.map(sub => ({
                id: sub.id,
                provider_id: sub.provider_id,
                provider_name: sub.provider_name,
                status: sub.status,
                items: typeof sub.items === 'string' ? JSON.parse(sub.items) : sub.items,
                price: sub.price
            }));
        } else {
            order.sub_orders = [];
...
// Create new order (with optional multi-provider splitting)
router.post('/', authenticatePartner, async (req, res) => {
    ...
    try {
        ...
        const {
            ...
            orderType // 'app' or 'manual' — sent by frontend
        } = req.body;

        // Determine order_type: explicit 'app' from frontend, or infer from source
        ...
        // Ensure delivery address has a meaningful value
        const effectiveDeliveryAddress = (deliveryAddress && deliveryAddress.trim()) ? deliveryAddress.trim() : 'عنوان يدوي';

        // Get creator name for attribution
        let creatorName = '';
        try {
            const creatorRes = await pool.query('SELECT name FROM users WHERE id = $1', [userId]);
            creatorName = creatorRes.rows[0]?.name || '';
        } catch (e) { /* ignore */ }

        // Determine effective source: manual orders get creator attribution
        const effectiveSource = (source === 'manual' || !source)
            ? (creatorName ? `طلب يدوي بواسطة ${creatorName}` : 'manual')
            : source;
        ...
        // ═══════════════════════════════════════════════════════════
        // CHECK: Do any items have a providerId? If so, activate split mode.
        // ═══════════════════════════════════════════════════════════
        const itemsWithProvider = finalItems.filter(p => p.providerId);
        const splitMode = itemsWithProvider.length > 0;

        if (splitMode) {
            // ══════════ SPLIT MODE: Multi-Provider Routing ══════════
            console.log(`[Halan POST] 🔀 Split mode activated! ${itemsWithProvider.length} items assigned to providers.`);
            ...
            if (source !== 'qareeblak') {
                effectiveSupervisorId = (role === 'supervisor') ? userId : null;
                if (!effectiveSupervisorId) {
                    const supResult = await client.query("SELECT id FROM users WHERE user_type = 'partner_supervisor' LIMIT 1");
                    if (supResult.rows.length > 0) effectiveSupervisorId = supResult.rows[0].id;
                }
            }
            const effectiveCourierId = (role === 'courier' && !courierId) ? userId : (courierId || null);

            // Create parent_orders record (global order tracker)
            const totalPrice = finalItems.reduce((sum, p) => sum + ((parseFloat(p.price) || 0) * (parseInt(p.quantity) || 1)), 0);
            ...
            `, [
                userId,
                totalPrice,
                `عميل: ${customerName} | هاتف: ${customerPhone} | عنوان: ${effectiveDeliveryAddress}`,
                JSON.stringify({ customerName, customerPhone, deliveryAddress: effectiveDeliveryAddress, notes })
            ]);
            const parentId = parentRes.rows[0].id;

            // Create delivery_orders record (courier-facing, contains ALL items)
            const deliveryRes = await client.query(`
                ...
                RETURNING *
            `, [
                orderNumber, customerName, customerPhone, pickupAddress || 'المحل / المخزن', effectiveDeliveryAddress,
                pLat, pLng, dLat, dLng,
                effectiveCourierId, effectiveSupervisorId, 'pending', notes || null, dFee, JSON.stringify(finalItems), effectiveSource, effectiveOrderType
            ]);
            const deliveryOrder = deliveryRes.rows[0];

            // Group items by providerId
            const grouped = {};
            itemsWithProvider.forEach(item => {
                const pid = item.providerId;
                if (!grouped[pid]) {
                    grouped[pid] = { providerName: item.providerName || 'مقدم خدمة', items: [] };
                }
                grouped[pid].items.push(item);
            });

            // Use CUSTOMER name for bookings (not admin name)
            const bookingUserName = customerName || 'عميل';

            // ⚡ OPTIMIZATION: Create all bookings in PARALLEL with Promise.all
            const bookingPromises = Object.entries(grouped).map(([providerId, group]) => {
                const providerTotal = group.items.reduce((sum, item) =>
                    sum + ((parseFloat(item.price) || 0) * (parseInt(item.quantity) || 1)), 0);

                const detailsStr = `الهاتف: ${customerPhone} | العنوان: ${effectiveDeliveryAddress}${notes ? ' | ملاحظات: ' + notes : ''}`;
                ...
                `, [
                    ...
                    providerId,
                    bookingUserName,
                    `طلب يدوي (${group.items.length} أصناف)`,
                    group.providerName,
                    providerTotal,
                    detailsStr,
            ...
            const bookingIds = bookingResults.map(r => r.rows[0].id);

            // 🔓 COMMIT TRANSACTION (Data is now safe)
            await client.query('COMMIT');
            console.log(`[Halan POST] ✅ Split order created: Parent=#${parentId}, Delivery=#${deliveryOrder.id}, Bookings=[${bookingIds.join(',')}]`);

            // Auto-assign courier ONLY if no manual courier was selected
            let finalOrder = deliveryOrder;
            if (!effectiveCourierId && !courierId) {
                try {
                    const io = req.app.get('io');
                    const assignedCourier = await performAutoAssign(deliveryOrder.id, userId, io);
                    if (assignedCourier) {
                        const refreshed = await pool.query(`
                            ...
                            WHERE o.id = $1
                        `, [deliveryOrder.id]);
                        finalOrder = refreshed.rows[0];
                    }
                } catch (assignError) {
                    console.error(`[Halan] Auto-assign failed for split order #${deliveryOrder.id}:`, assignError.message);
                }
            } else {
                // Manual courier selected - fetch full order with courier name
                const refreshed = await pool.query(`
                    SELECT o.*, u.name as courier_name, u.phone as courier_phone
                    FROM delivery_orders o
                    LEFT JOIN users u ON o.courier_id = u.id
                    WHERE o.id = $1
                `, [deliveryOrder.id]);
                if (refreshed.rows.length > 0) finalOrder = refreshed.rows[0];
            }

            // 📤 SEND RESPONSE IMMEDIATELY (UI stops loading NOW)
            res.status(201).json({
                success: true,
                data: finalOrder,
                parentId: parentId,
                ...
                splitMode: true,
                message: `تم تقسيم الطلب إلى ${bookingIds.length} طلبات فرعية`
            });

            // 🔥 FIRE & FORGET: Send notifications in the background (non-blocking)
            setImmediate(async () => {
                try {
                    const io = req.app.get('io');
                    const { createNotification } = require('./notifications');

                    const notificationPromises = Object.entries(grouped).map(async ([providerId, group]) => {
                        try {
                            const provUserRes = await pool.query('SELECT user_id FROM providers WHERE id = $1', [providerId]);
                            if (provUserRes.rows.length > 0 && provUserRes.rows[0].user_id) {
                                await createNotification(
                                    provUserRes.rows[0].user_id,
                                    `📦 طلب جديد! ${group.items.length} أصناف بانتظار التحضير`,
                                    'new_order',
                                    parentId,
                                    io
                                );
                            ...
                            console.error(`[Notification Error] Provider ${providerId}:`, err.message);
                        }
                    });

                    await Promise.all(notificationPromises);
                    console.log(`[Halan POST] 🔔 Sent ${notificationPromises.length} notifications`);
                } catch (notifError) {
                    console.error('[Halan POST] Background notification error:', notifError.message);
                }
            });

        } else {
            // ══════════ NORMAL MODE: Single delivery order (existing flow) ══════════
            const orderNumber = `HLN-${Date.now().toString(36).toUpperCase()}`;
            ...
            // Respect manually selected courier - DON'T override
            const effectiveCourierId = (role === 'courier' && !courierId) ? req.user.userId : (courierId || null);
            ...
            `, [
                orderNumber, customerName, customerPhone, pickupAddress || 'المحل / المخزن', effectiveDeliveryAddress,
                pLat, pLng, dLat, dLng,
                effectiveCourierId, effectiveSupervisorId || null, initialStatus, notes || null, dFee, JSON.stringify(finalItems), effectiveSource, effectiveOrderType
            ]);

            let newOrder = result.rows[0];

            // Auto-assign ONLY if no manual courier was selected AND this is an APP order
            // Manual orders: courier is already assigned by creator, no auto-assign needed
            if (!effectiveCourierId && !courierId && effectiveOrderType === 'app') {
                try {
                    ...
                    console.error(`[Halan] Auto-assign failed for order #${newOrder.id}:`, assignError.message);
                }
            }
        ...
        // 🔄 ROLLBACK on error (prevent corrupt data)
        if (client) {
            try {
                await client.query('ROLLBACK');
                console.log('[Halan POST] ⚠️ Transaction rolled back');
            } catch (rollbackError) {
                console.error('[Halan POST] Rollback failed:', rollbackError.message);
            }
        }
        console.error('Create order error:', error);
        res.status(500).json({ success: false, error: 'حدث خطأ في السيرفر', details: error.message });
    } finally {
...
router.put('/:id', authenticatePartner, async (req, res) => {
    try {
        ...
        if (status) {
            ...
            // AUTO-ASSIGN CHECK: Only for APP orders without a courier.
            // MANUAL orders already have a courier assigned by the creator — skip auto-assign.
            const currentOrderCheck = await pool.query('SELECT courier_id, order_type, source FROM delivery_orders WHERE id = $1', [id]);
            if (currentOrderCheck.rows.length > 0 && !currentOrderCheck.rows[0].courier_id) {
                const orderRow = currentOrderCheck.rows[0];
                const isManualOrder = orderRow.order_type === 'manual' || (orderRow.source && !orderRow.source.includes('qareeblak'));

                if (isManualOrder) {
                    console.log(`[Halan] ⏭️ Manual order #${id} has no courier but skipping auto-assign (manual flow).`);
                } else {
                    console.log(`[Halan] 🚀 App Order #${id} (Status: ${status}) needs a courier. Triggering auto-assignment...`);
                    try {
                        const io = req.app.get('io');
                        await performAutoAssign(id, userId, io, status === 'pending' ? 'assigned' : status);


                        // If assigned successfully, return early
                        return res.json({ success: true, message: 'تم تعيين المندوب وتحديث الحالة' });
                    } catch (e) {
                        console.error('[Halan] Auto-assign failed during status update:', e.message);
        ...
        // ===========================================
        // 🔄 SMART RE-ROUTING & CLEANUP LOGIC
        // ===========================================
        console.log(`[Edit Order] Processing edit for Order #${id}...`);

        // 1. Get current order details
        const currentOrderResult = await pool.query('SELECT * FROM delivery_orders WHERE id = $1', [id]);
        if (currentOrderResult.rows.length === 0) return res.status(404).json({ success: false, error: 'Order not found' });
        const currentOrder = currentOrderResult.rows[0];

        // 2. Identify Item Changes & Provider Shifts
        // We will process the NEW item list and reorganize bookings accordingly
        const newItems = items || [];
        const itemsByProvider = {};

        // Group new items by provider
        newItems.forEach(item => {
            const pid = item.providerId || 'generic'; // 'generic' means no specific provider (default to delivery order items)
            if (!itemsByProvider[pid]) itemsByProvider[pid] = { items: [], name: item.providerName };
            itemsByProvider[pid].items.push(item);
        });

        // 3. Sync Bookings (Sub-Orders)
        // Find ALL bookings linked to this parent (if any) or linked via halan_order_id
        // NOTE: If this was a manual order, it might link to bookings via halan_order_id
        const linkedBookingsRes = await pool.query('SELECT id, provider_id, parent_order_id FROM bookings WHERE halan_order_id = $1', [id]);
        const existingBookingsMap = {}; // provider_id -> booking_id
        linkedBookingsRes.rows.forEach(b => existingBookingsMap[b.provider_id] = b.id);

        console.log(`[Edit Order] Existing bookings:`, existingBookingsMap);
        console.log(`[Edit Order] New Groups:`, Object.keys(itemsByProvider));

        // Process Groups
        for (const [providerId, group] of Object.entries(itemsByProvider)) {
            if (providerId === 'generic') continue; // These stay on the main delivery order JSON

            const bookingId = existingBookingsMap[providerId];
            const groupTotal = group.items.reduce((sum, i) => sum + ((Number(i.price) || 0) * (Number(i.quantity) || 1)), 0);

            if (bookingId) {
                // UPDATE existing booking
                console.log(`[Edit Order] Updating Booking #${bookingId} for Provider ${providerId}`);
                await pool.query(`
                    UPDATE bookings 
                    SET items = $1, price = $2, details = $3, updated_at = NOW()
                    WHERE id = $4
                `, [
                    JSON.stringify(group.items),
                    groupTotal,
                    `تعديل: ${customerName} | ${customerPhone} | ${deliveryAddress}`,
                    bookingId
                ]);
                // Remove from map so we know it's handled
                delete existingBookingsMap[providerId];
            } else {
                // CREATE new booking (Item moved to NEW provider)
                console.log(`[Edit Order] Creating NEW Booking for Provider ${providerId}`);
                // Verify provider exists
                const provRes = await pool.query('SELECT name FROM providers WHERE id = $1', [providerId]);
                const provName = provRes.rows[0]?.name || group.name || 'مقدم خدمة';

                // Try to get parent_id from existing links or create one? 
                // For manual orders, usually we check if there's a parent.
                // If not, we might need to assume this delivery order IS the aggregator.
                // Let's use the current delivery order ID as link.

                const newBookingRes = await pool.query(`
                    INSERT INTO bookings 
                    (user_id, provider_id, user_name, service_name, provider_name, price, details, items, status, halan_order_id, parent_order_id)
                ...
                `, [
                    ...
                    provName,
                    groupTotal,
                    `تعديل: ${customerName} | ${customerPhone}`,
                    JSON.stringify(group.items),
                    id,
                    linkedBookingsRes.rows[0]?.parent_order_id || null
                ]);
                const newBookingId = newBookingRes.rows[0].id;

                // 🔔 Trigger Notification for the NEW Provider
                try {
                    const provUserRes = await pool.query('SELECT user_id FROM providers WHERE id = $1', [providerId]);
                    if (provUserRes.rows.length > 0 && provUserRes.rows[0].user_id) {
                        const { createNotification } = require('./notifications');
                        ...
                        await createNotification(
                            provUserRes.rows[0].user_id,
                            `🛒 تم إضافة أصناف جديدة لطلبك! (${group.items.length} صنف)`,
                            'new_order', // or 'order_update'
                            newBookingId, // Link to the booking
                            io
                        );
                        console.log(`[Edit Order] 🔔 Notification sent to Provider ${providerId}`);
                    }
                } catch (notifErr) {
                    console.error('[Edit Order] Failed to send notification:', notifErr.message);
                }
            }
        }

        // 4. HARD DELETE Cleanup
        // Any provider in existingBookingsMap that wasn't matched in itemsByProvider constitutes an EMPTY order now
        for (const [pId, bId] of Object.entries(existingBookingsMap)) {
            console.log(`[Edit Order] 🗑️ HARD DELETE: Booking #${bId} (Provider ${pId}) is now empty.`);
            await pool.query('DELETE FROM bookings WHERE id = $1', [bId]);
        }

        // 5. Recalculate Parent Order Total (if applicable)
        // We try to find a parent_order_id from the linked bookings we fetched earlier
        const parentId = linkedBookingsRes.rows[0]?.parent_order_id;
        if (parentId) {
            console.log(`[Edit Order] Recalculating total for Parent Order #${parentId}...`);
            await pool.query(`
                UPDATE parent_orders
                SET total_price = (SELECT COALESCE(SUM(price), 0) FROM bookings WHERE parent_order_id = $1),
                    updated_at = NOW()
                WHERE id = $1
            `, [parentId]);
        }

        // 5. Update Main Delivery Order
        // The main order keeps the FULL list of items for the courier
        ...
        await pool.query(`
            ...
            WHERE id = $9
        `, [customerName, customerPhone, deliveryAddress, deliveryFee, JSON.stringify(newItems), notes, courierId || null, targetStatus, id]);
        ...
        res.json({ success: true, message: 'تم تعديل الطلب وإعادة توزيع الأصناف بنجاح' });
