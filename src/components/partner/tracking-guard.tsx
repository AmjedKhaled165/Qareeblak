...
export function TrackingGuard({ children }: { children: React.ReactNode }) {
    ...
    // Determine if we should block the UI
    const shouldBlock = userRole === 'courier' && (!isTracking || isExpired);
    ...
    return (
        <div className="relative min-h-screen">
            ...
            <div className={shouldBlock ? "blur-sm pointer-events-none select-none h-screen overflow-hidden" : ""}>
                {children}
            </div>
        </div>
    );
