    ...
    const handleUpdateOrder = async (id: number, payload: any) => {
        try {
            if (Object.prototype.hasOwnProperty.call(payload, 'courier_id')) {
                ...
                await apiCall(`/halan/orders/${id}/assign-courier`, { method: 'PATCH', body: JSON.stringify({ courierId }) });
            } else {
                await apiCall(`/halan/orders/${id}/meta`, { method: 'PATCH', body: JSON.stringify(payload) });
            }

            if (selectedOrder && selectedOrder.id === id) {
                const updatedOrder = { ...selectedOrder, ...payload };
                if (payload.courier_id !== undefined) updatedOrder.courier_name = drivers.find(d => d.id == payload.courier_id)?.name || null;
                if (payload.supervisor_id !== undefined) updatedOrder.supervisor_name = managers.find(m => Number(m.id) === Number(payload.supervisor_id))?.name || null;
                        ...
                        return (
                                ...
                                onClick={async () => {
                                    ...
                                    try {
                                        ...
                                        if (full?.success && full?.data) {
                                            ...
                                            if (typeof enriched.items === 'string') { try { enriched.items = JSON.parse(enriched.items); } catch { enriched.items = []; } }
                            ...
                            >
                                ...
                                <div className="flex justify-between items-start mb-3 pr-3">
                                    <div>
                                        <div className="flex items-center gap-2 mb-1"><StatusIcon className="w-4 h-4 text-slate-500" /><span className="font-bold text-slate-800 dark:text-slate-100">{order.customer_name} #{order.display_id || order.id}</span></div>
                                        <div className="flex items-center gap-2">
                                            <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${getSourceColor(order.source)}`}>{mapSourceLabel(order.source)}</span>
                                            <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${getStatusStyle(order.status)}`}>{getStatusLabel(order.status)}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="flex justify-between items-center pt-2 border-t border-slate-100 dark:border-slate-700 pr-3">
                                    <span className="text-xs text-slate-400">{new Date(order.created_at).toLocaleDateString('ar-EG')}</span>
                                    {(() => {
                                        const items = Array.isArray(order.items) ? order.items : (typeof order.items === 'string' ? JSON.parse(order.items || '[]') : []);
